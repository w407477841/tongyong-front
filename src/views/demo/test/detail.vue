...
<script>
import {insertDemoTest,updateDemoTest,getDemoTestById} from './api.js';
...
export default {
  ...
  methods: {
    ...
    getInfo() {
      ...
      getDemoTestById(param).then(res => {
        ...
        if (valid) {
          this.$confirm('确认保存吗？', '提示', {
            ...
            if (this.openType == 'insert') {
              method = insertDemoTest(this.params);
            } else if (this.openType == 'update') {
              method = updateDemoTest(this.params);
            }
            method.then((res) => {
              if(res.code == 200) {
                this.$message({
