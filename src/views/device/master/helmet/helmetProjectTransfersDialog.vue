    ...
    export default {
        ...
        data() {
            return {
                ...
                projectId: "",
                formDatasRules: {
                    projectId: {required: true, message: "请选择工程名称", trigger: "blur"},
                },
                projectList: [],
                edit: false,
                //dialog模式
        ...
        methods: {
            ...
            handleSubmit() {
                this.$refs.editForm.validate(valid => {
                    if (valid) {
                        this.$confirm('确认将选中的安全帽进行项目调拨吗？', '提示', {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning'
                        }).then(() => {
                            this.params.status = 0;
                            projectTransfersApi(this.params).then(res => {
                                if (res.success === true) {
