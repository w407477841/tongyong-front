    ...
    <div class="container-box condition-box">
      <div class="conditionBar">
        <el-form :inline="true" class="condition-form">
          <el-form-item>
            <el-input size="mini" v-model="keyword" :disabled="edit" placeholder="订单号 | 手机号" :clearable="true"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="订单状态" prop="status" width="180" :formatter="fmtStatus" ></el-table-column>
...
<script>
import {getBalanceRecords,deleteBalanceRecords} from './api.js';
...
export default {
  data() {
    return {
      ...
      id : '',
      certificateTypeList : [],
      operationPrex : 'balance:balancerecord'
    };
  },
  computed: {
    params() {
  ...
  methods: {
    ...
    //获取列表
    getListData() {
      getBalanceRecords(this.params).then(res => {
    ...
    handleDelete(sels) {
      this.$confirm("确认删除选中记录吗？","提示", {type: "warning"}).then(() => {
        ...
        deleteBalanceRecords(para).then((res) => {
          if(res.code == 200) {
            this.$message({
              type : 'success',
              message : res.message
