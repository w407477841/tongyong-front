...
<script>
import {insertTargetSetWater,updateTargetSetWater,selectTargetSetWaterById} from "@/views/information/api/targetSetWater.js";
export default {
  data() {
    return {
      labelWidth: "150px",
  ...
  methods: {
        ...
        if (valid) {
          this.$confirm("确认保存吗？", "提示", {
            ...
            if (this.routeType == "insert") {
              method = insertTargetSetWater(this.params);
            } else if (this.routeType == "update") {
              this.params.body.id = this.detailsId;
              method = updateTargetSetWater(this.params);
            }
            method.then((res) => {
              if(res.code == 200) {
                this.$message({
    ...
    getInfo() {
      ...
      selectTargetSetWaterById(param).then(res => {
