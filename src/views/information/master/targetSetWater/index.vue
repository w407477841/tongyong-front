        ...
        <div class="container-box list-box">
            <div class="table-box">
                <el-table size="small" :data="listData" style="width: 100%" height="100%"  highlight-current-row @selection-change="listSelectionChange"  header-row-class-name="xyTableHeader">
                    ...
                    <el-table-column label="每秒耗水量(立方米)" prop="dissipation"></el-table-column>
...
<script>
import {selectTargetSetWater,deleteTargetSetWater} from "@/views/information/api/targetSetWater.js";
...
export default {
  ...
  data() {
    return {
      operationPrex: "electric:infoMain:targetSetWater",
      listData: [],
      isDangerous: "",
      keyword: "",
      pageIndex: 1,
  ...
  methods: {
    ...
    //加载列表
    getListData() {
      selectTargetSetWater(this.params).then(res => {
    ...
    handleDelete(sels) {
      ...
      this.$confirm("确认删除选中记录吗？", "提示", {
        ...
        deleteTargetSetWater(params).then(res => {
          this.$message({
            message: res.message,
