<template>
  <div class="app-container list-container">
    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="总预警(条)" prop="alarmAll"></el-table-column>
          <el-table-column label="重量预警(条)" prop="alarmWeight"></el-table-column>
          <el-table-column label="幅度预警(条)" prop="alarmRange"></el-table-column>
          <el-table-column label="力矩预警(条)" prop="alarmMoment"></el-table-column>
          <el-table-column label="碰撞预警(条)" prop="alarmCollision"></el-table-column>
          <el-table-column label="倾斜报警(条)" prop="alarmHeight"></el-table-column>
    ...
    <el-dialog title="预警详情" :visible.sync="promptingDetailVisible" :close-on-click-modal="false" width="60%" v-if="promptingDetailVisible">
      <prompting-message-detail :id="this.id" @setShow="setShow" :keyword="this.keyword"></prompting-message-detail>
    </el-dialog>

    <el-dialog title="发送短信" :visible.sync="promptingAlarmVisible" :close-on-click-modal="false" width="60%" v-if="promptingAlarmVisible">
      <prompting-message-alarm :id="this.id" @setShow="setShow" :content="this.content"></prompting-message-alarm>
    </el-dialog>
  </div>
</template>
...
import {selectPromptingAlarm} from '@/views/api/equipmentApi.js';
import PromptingMessageDetail from './promptingMessageDetail';
import PromptingMessageAlarm from './promptingMessageAlarm';
...
export default {
  data() {
    return {
      ...
      endDate : '',
      edit : false,
      promptingDetailVisible : false,
      promptingAlarmVisible : false,
      id : '',
      content : '',
      mainInfo : '',
      dateShow : true,
  ...
  methods: {
    ...
    //获取列表
    getListData() {
      selectPromptingAlarm(this.params).then(res => {
        if(res.data.list[0].alarmAll == 0) {
          this.listData = [];
        }else {
    ...
    goToCheckDetail : function(row) {
      ...
      this.keyword = "预警";
      this.promptingDetailVisible = true;
    },
    //点击短信提醒按钮弹出短信页面
    goToMessageAlarm : function(row) {
      this.id = row.id;
      this.content = "工程名称：" + this.mainInfo.projectName + ",黑匣子编号：" + this.mainInfo.deviceNo + ",总预警：" + row.alarmAll + ",重量预警：" + row.alarmWeight + ",幅度预警：" + row.alarmRange + ",力矩预警 ：" + row.alarmMoment + ",碰撞预警：" + row.alarmCollision;
      this.promptingAlarmVisible = true;
    },
    //子组件向父组件传值
    setShow : function(data) {
      if(data.sign == "关闭信息") {
        this.promptingAlarmVisible = false;
      }
    },
    //选择查询条件时改变日期选择器的可用性
    dateChange : function() {
    ...
    goBack : function() {
      this.$router.push({ path:  '/realTimeMonitoring' });
    }
  },
  components : {
    PromptingMessageDetail,
    PromptingMessageAlarm
  }
};
</script>
