<template>
  <div class="app-container list-container">
    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="超载违章(条)" prop="alarmWeight"></el-table-column>
    ...
    <el-dialog title="违章详情" :visible.sync="violationDetailVisible" :close-on-click-modal="false" width="60%" v-if="violationDetailVisible">
      <violation-information-detail :id="this.id" @setShow="setShow" :keyword="this.keyword"></violation-information-detail>
    </el-dialog>

    <el-dialog title="发送短信" :visible.sync="violationAlarmVisible" :close-on-click-modal="false" width="60%" v-if="violationAlarmVisible">
      <violation-information-alarm :id="this.id" @setShow="setShow" :content="this.content"></violation-information-alarm>
    </el-dialog>
  </div>
</template>
...
import {selectViolationAlarm} from '@/views/api/equipmentApi.js';
import ViolationInformationDetail from './violationInformationDetail';
import ViolationInformationAlarm from './violationInformationAlarm';
...
export default {
  data() {
    return {
      ...
      endDate : '',
      edit : false,
      violationDetailVisible : false,
      violationAlarmVisible : false,
      id : '',
      content : '',
      mainInfo : '',
      dateShow : true,
  ...
  methods: {
    ...
    //获取列表
    getListData() {
      selectViolationAlarm(this.params).then(res => {
        if(res.data.list[0].alarmWeight == 0) {
          this.listData = [];
        }else {
    ...
    goToCheckDetail : function(row) {
      ...
      this.keyword = "违章";
      this.violationDetailVisible = true;
    },
    //点击短信提醒按钮弹出短信页面
    goToMessageAlarm : function(row) {
      this.id = row.id;
      this.content = "工程名称：" + this.mainInfo.projectName + ",黑匣子编号：" + this.mainInfo.deviceNo +  ",重量违章：" + row.alarmWeight;
      this.violationAlarmVisible = true;
    },
    //子组件向父组件传值
    setShow : function(data) {
      if(data.sign == "关闭信息") {
        this.violationAlarmVisible = false;
      }
    },
    //选择查询条件时改变日期选择器的可用性
    dateChange : function() {
  ...
  components : {
    ViolationInformationDetail,
    ViolationInformationAlarm
  }
};
</script>
