<template>
  <div class="app-container list-container">
    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="总报警(条)" prop="alarmAll"></el-table-column>
          <el-table-column label="重量报警(条)" prop="alarmWeight"></el-table-column>
          <el-table-column label="幅度报警(条)" prop="alarmRange"></el-table-column>
          <el-table-column label="力矩报警(条)" prop="alarmMoment"></el-table-column>
          <el-table-column label="碰撞报警(条)" prop="alarmCollision"></el-table-column>
          <el-table-column label="风速报警(条)" prop="alarmSpeed"></el-table-column>
          <el-table-column label="倾斜报警(条)" prop="alarmTiltAngle"></el-table-column>
    ...
    <el-dialog title="报警详情" :visible.sync="warningDetailVisible" :close-on-click-modal="false" width="60%" v-if="warningDetailVisible">
      <warning-message-detail :id="this.id" @setShow="setShow" :keyword="this.keyword"></warning-message-detail>
    </el-dialog>

    <el-dialog title="发送短信" :visible.sync="warningAlarmVisible" :close-on-click-modal="false" width="60%" v-if="warningAlarmVisible">
...
import {selectWarningAlarm} from '@/views/api/equipmentApi.js';
...
export default {
  ...
  methods: {
    ...
    goToMessageAlarm : function(row) {
      this.id = row.id;
      this.content = "工程名称：" + this.mainInfo.projectName + ",黑匣子编号：" + this.mainInfo.deviceNo + ",总报警：" + row.alarmAll + ",重量报警：" + row.alarmWeight + ",幅度报警：" + row.alarmRange + ",力矩报警 ：" + row.alarmMoment + ",碰撞报警：" + row.alarmCollision + ",风速报警：" + row.alarmSpeed + ",倾斜报警：" + row.alarmTiltAngle;
