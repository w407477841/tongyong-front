                ...
                <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row
                    ...
                    <el-table-column label="操作" width="300" align="center" fixed="right">
                        <template slot-scope="scope">
                            <electricButton @monitorStatus="monitorStatus" @operationData="operationData"
                                        @warningMessage="warningMessage" :row="scope.row"></electricButton>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
<script>
import { getListApi } from "@/views/information/api/electric.js";
import { getProjectListApi } from "@/views/device/api/crane.js";
import electricButton from "./electricButton";

export default {
  components: { electricButton },
  data() {
    return {
      listData: [],
      isDangerous: "",
  ...
  methods: {
    ...
    monitorStatus(sels) {
      this.$router.push({
        path: this.$route.path + "/deviceStatus",
        query: {
          id: sels.id,
