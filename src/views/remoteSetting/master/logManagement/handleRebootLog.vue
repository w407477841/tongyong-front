<template>
    <el-dialog width="80%" :title="title" :visible.sync="visibleRebootLog" append-to-body>
            ...
            <div class="container-box list-box">
                <div class="table-box">
                    <el-table size="small" :data="listData" style="width: 100%" highlight-current-row
                        ...
                        <el-table-column label="重启时间" prop="restartTime" :formatter="handleDate"></el-table-column>
                        <el-table-column label="重启人" prop="createUserName"></el-table-column>
                    </el-table>
                </div>
...
<script>
    import {getRebootLogApi} from "@/views/remoteSetting/api/logManagement.js";
    ...
    export default {
        ...
        watch: {
            ...
            visible: {
                handler: function (val, oldVal) {
                    this.visibleRebootLog = val;
                },
                immediate: true
            },
            visibleRebootLog(val) {
                if (!val) {
                    this.$emit('close', {});
                }
            }
        ...
        methods: {
            ...
            //获取数据列表
            getListData() {
                getRebootLogApi(this.params).then(res => {
