    ...
    <div class="container-box condition-box">
      <div class="conditionBar">
        <el-form :inline="true" class="condition-form">
          <el-form-item>
            <el-input size="mini" v-model="keyword"  placeholder="设备号 | 二维码" :clearable="true"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="二维码" prop="qrcode" width="180"></el-table-column>
          <el-table-column label="设备号" prop="deviceNo" width="180"></el-table-column>
          <!-- <el-table-column label="二维码地址" prop="qrcodeUrl" width="180"></el-table-column> -->
...
<script>
import {getBasicQrcodes,deleteBasicQrcodes} from './api.js';
...
export default {
  data() {
    return {
      ...
      id : '',
      certificateTypeList : [],
      operationPrex : 'basic:basicqrcode'
    };
  },
  computed: {
    params() {
  ...
  methods: {
    ...
    //获取列表
    getListData() {
      getBasicQrcodes(this.params).then(res => {
    ...
    handleDelete(sels) {
      this.$confirm("确认删除选中记录吗？","提示", {type: "warning"}).then(() => {
        ...
        deleteBasicQrcodes(para).then((res) => {
          if(res.code == 200) {
            this.$message({
              type : 'success',
              message : res.message
