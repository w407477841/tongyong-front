    ...
    <div class="container-box condition-box">
      <div class="conditionBar">
        <el-form :inline="true" class="condition-form">
          <el-form-item>
            <el-input size="mini" v-model="keyword"  placeholder="站点名称" :clearable="true"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="站点名称" prop="name" width="180"></el-table-column>
          <el-table-column label="地址" prop="address" width="180"></el-table-column>
          <el-table-column label="坐标" prop="coordinate" width="180"></el-table-column>
          <el-table-column label="备注" prop="comments" width="180"></el-table-column>
          <el-table-column label="物业" prop="orgName" width="180"></el-table-column>
...
<script>
import {getBasicStations,deleteBasicStations} from './api.js';
...
export default {
  data() {
    return {
      ...
      id : '',
      certificateTypeList : [],
      operationPrex : 'basic:basicstation'
    };
  },
  computed: {
    params() {
  ...
  methods: {
    ...
    //获取列表
    getListData() {
      getBasicStations(this.params).then(res => {
    ...
    handleDelete(sels) {
      this.$confirm("确认删除选中记录吗？","提示", {type: "warning"}).then(() => {
        ...
        deleteBasicStations(para).then((res) => {
          if(res.code == 200) {
            this.$message({
              type : 'success',
              message : res.message
