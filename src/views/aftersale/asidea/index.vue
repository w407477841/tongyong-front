    ...
    <div class="container-box list-box">
      <div class="table-box">
        <el-table size="small" :data="listData" style="width: 100%" height="100%" highlight-current-row @selection-change="listSelectionChange" header-row-class-name="xyTableHeader">
          ...
          <el-table-column label="反馈状态" prop="feedbackStatus" width="180" :formatter="fmtStatusFeedback"></el-table-column>
          <el-table-column label="处理状态" prop="status" width="180" :formatter="fmtStatus"></el-table-column>
          <el-table-column label="意见内容" prop="ideaContent" width="180"></el-table-column>
          <el-table-column label="处理内容" prop="feedbackContent" width="180"></el-table-column>
...
<script>
import {getAsIdeas,deleteAsIdeas} from './api.js';
...
export default {
  data() {
    return {
      ...
      id : '',
      certificateTypeList : [],
      operationPrex : 'aftersale:asidea'
    };
  },
  computed: {
    params() {
  ...
  methods: {
    ...
    //获取列表
    getListData() {
      getAsIdeas(this.params).then(res => {
    ...
    //反馈状态
    fmtStatusFeedback :function(row){
      return row.feedbackStatus == 0 ? "未解决" : "已解决"
    }
  },
  components : {
    Detail
